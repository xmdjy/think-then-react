...
"""
TTR authors: HF T5 does not support src&memory causal mask, so we have to modify it.
"""
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if position_bias is None:
            ...
            if src_causal_mask is not None:
                position_bias = position_bias + src_causal_mask  # (batch_size, n_heads, src_length, src_length)
            if memory_causal_mask is not None:
                position_bias = position_bias + memory_causal_mask  # (batch_size, n_heads, tgt_length, src_length)
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.layer[0](
            ...
            src_causal_mask=src_causal_mask if not self.is_decoder else None
        )
