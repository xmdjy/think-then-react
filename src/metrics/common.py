...
def calculate_diversity(activations, diversity_times):
    assert len(activations.shape) == 2
    assert activations.shape[0] > diversity_times
    num_samples = activations.shape[0]

    first_indices = np.random.choice(num_samples, diversity_times, replace=False)
    second_indices = np.random.choice(num_samples, diversity_times, replace=False)
    dist = linalg.norm((activations[first_indices] - activations[second_indices])/2, axis=1)
    return dist.mean()
...
def calculate_fid(gt_embeddings, pred_embeddings):
    gt_mu, gt_cov = calculate_activation_statistics(gt_embeddings)
    pred_mu, pred_cov = calculate_activation_statistics(pred_embeddings)
